...
| `YANDEX_WEBMASTER_TOKEN` | Серверный OAuth-токен API Яндекс Вебмастера для статусов и переобхода |
| `INDEXNOW_KEY` | Ключ IndexNow (8–128 символов `a-zA-Z0-9-`) для уведомлений об изменениях |
...
- **Админка → Индексация** сопоставляет все опубликованные канонические URL с данными API Яндекс Вебмастера, показывает причины исключения и формирует приоритетный список для суточной квоты переобхода.
- После публикации, изменения или удаления публичного контента приложение автоматически отправляет затронутые URL в IndexNow. Массовый первичный импорт через IndexNow намеренно не выполняется: для него используются sitemap и контролируемый переобход.

### Подключение Яндекс Вебмастера

1. Получите OAuth-токен по [инструкции Яндекса](https://yandex.ru/dev/webmaster/doc/ru/tasks/how-to-get-oauth) в аккаунте, где подтверждены права на домен. Для приложения нужны разрешения `webmaster:hostinfo` и `webmaster:verify`.
2. Задайте на сервере `YANDEX_WEBMASTER_TOKEN`. Токен не должен иметь префикс `NEXT_PUBLIC_`.
3. Сгенерируйте ключ командой `node -e "console.log(require('node:crypto').randomBytes(24).toString('hex'))"` и задайте его как `INDEXNOW_KEY`.
4. После деплоя проверьте, что `/indexnow-key.txt` возвращает ключ, затем откройте **Админка → Индексация**. На странице можно выбрать только URL, которые приложение заново сверило с опубликованным контентом сайта.
