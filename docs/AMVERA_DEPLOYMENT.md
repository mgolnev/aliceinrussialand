...
постоянный серверный обработчик: он разбирает остальную очередь по одной задаче,
проверяет её каждые 30 секунд и возобновляет обработку после перезапуска контейнера.
Нужен `CRON_SECRET` в переменных приложения. В админке есть ручной запуск всей пачки.
Внешний cron теперь только резерв; подробности — в [AMVERA_SEO_CRON.md](./AMVERA_SEO_CRON.md).
