...
function post(id: string, variantsJson = '{"w1280":"/large.webp","w640":"/small.webp"}') {
  return { post: { id, slug: id, title: "Новая публикация", images: [{ variantsJson, alt: "Рисунок ручкой", width: 800, height: 1000 }] } };
}
...
describe("публичный каталог прогулки", () => {
  ...
  it("объединяет повторные работы и сохраняет связи между подборками", async () => {
    ...
    expect(catalogue.posts[0].image).toMatchObject({ src: "/large.webp", thumbnail: "/small.webp" });
  });
